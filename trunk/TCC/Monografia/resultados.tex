...
Como o AppMan é um sistema em desenvolvimento, já citado algumas vezes no texto, sua execução nem sempre era o que esperávamos. A instalação do ambiente EXEHDA demandou um tempo não contabilizado no cronograma. A dependência do \emph{middleware} EXEHDA também causou problemas. Inúmeros erros encontrados no momento dos teste foi devido ao EXEHDA junto com o servidor LDAP. Em particular a necessidade de um servidor LDAP funcionando de acordo com o esperado foi bastante difícil de obter. Isso deve-se ao fato do \emph{middleware ser relativamente antigo se comparado com as versões de OpenLDAP disponíveis.
